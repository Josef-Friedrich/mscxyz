...
- Add the new class `VboxText` to be able to remove the style overrides,
  replace the HTML text with a plain text version and set the style name
  of the text elements in a `vbox`.
- Add new properties in the class `Vbox`: `title_element`, `subtitle_element`,
  `composer_element`, `lyricist_element` and `composer_element` of datatype
  `VboxText`.

### Fixed

- The style name for the lyricist text element in the vbox is now `poet`
...
- Add new options: `--part-name`, `--instrument-excerpt`
