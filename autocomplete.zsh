...
_shtab_musescore_manager_options=(
  ...
  "*"{-i,--distribute-fields}"[Distribute source fields to target fields applying a format string     on the source fields. It is possible to apply multiple     --distribute-fields options. SOURCE_FIELDS can be a single field or a     comma separated list of fields\: field_one,field_two. The program     tries first to match the FORMAT_STRING on the first source field. If this    fails, it tries the second source field ... an so on.]:meta_dist:"
  ...
  "(*)::Path to a \*.msc\[zx\]\" file     or a folder which contains \"\*.msc\[zx\]\" files. In conjunction     with the subcommand \"help\" this positional parameter     accepts the names of all other subcommands or the word     \"all\".:"
)
